...
from helpers import lum, ang_mom_vector, get_spherical_from_cartesian, compute_kappa
...
def get_data(ii, tag, inp = 'FLARES'):
    ...
    with h5py.File(sim, 'r') as hf:
        ...
        mstar = np.array(hf[num+tag+'/Galaxy/Mstar_aperture'].get('30'), dtype = np.float64)*1e10
        cop_vel = np.array(hf[num+tag+'/Galaxy'].get('Velocity'), dtype = np.float64)
        DTMs = np.array(hf[num+tag+'/Galaxy'].get('DTM'), dtype = np.float64)
    ...
    return cop, mstar, cop_vel, DTMs, S_coords, G_coords, S_mass, S_mass_curr, G_mass, S_Z, S_age, S_vel, G_vel, G_Z, begin, end, gbegin, gend
...
cop, mstar, cop_vel, DTMs, S_coords, G_coords, S_mass, S_mass_curr, G_mass, S_Z, S_age, S_vel, G_vel, G_Z, begin, end, gbegin, gend = get_data(num, tag, inp = 'FLARES')
...
#jj = req_ind[4]
for kk, jj in enumerate(req_ind[0:1]):
    ...
    this_svel = S_vel[begin[jj]:end[jj]].T - cop_vel[:,jj]

    this_gmass = G_mass[gbegin[jj]:gend[jj]]
    this_gvel = G_vel[gbegin[jj]:gend[jj]].T - cop_vel[:,jj]
    ...
    Kco, Krot = compute_kappa(this_smass_curr, scoords, this_svel)
    ...
    with h5py.File(F'../data2/FLARES_{region}_data.hdf5', 'r') as hf1:
    ...
    plt.savefig(F'old_test_3.png', dpi = 300, bbox_inches='tight')
